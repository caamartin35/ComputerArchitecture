...
void pipe_cycle()
{
    ...
    printf("PIPE: inst_stall: %d\n", pipe.inst_stall);
    printf("PIPE: data_stall: %d\n", pipe.data_stall);
#endif
    printf("PIPE: BEFORE CYCLE\n");
    pipe_stage_wb();
    pipe_stage_mem();
    pipe_stage_execute();
    pipe_stage_decode();
    ...
    //update l2 cycles and check for fill notifications
    uint32_t pc_mem_addr;
    l2_update_cycles();
    if ((pc_mem_addr=l2_miss_handler()) != 0){
        //printf("PIPE: ")
        if ((pc_mem_addr >> 5) == (pipe.PC >> 5)){
            pipe.inst_stall = 0;
            inst_load(pc_mem_addr);
        }
        else if (pipe.mem_op != NULL && (pc_mem_addr >> 5) == ((pipe.mem_op)->mem_addr >> 5)){
            pipe.data_stall = 0;
            data_load(pc_mem_addr);
        }
    }
    ...
    if ((request = schedule_request()) != NULL){
      printf("PIPE: Controller Scheduled a request: Bank=%d, Row=%d, Time=%d, Source=%d\n", request->bank, request->row, request->time, request->source);
...
void pipe_stage_mem()
{
    ...
    if (op->is_mem){
        if (!pipe.data_stall){
            if (!data_check(op->mem_addr))
                pipe.data_stall = 1;
        }
        else{
            if (l2_data_hit_handler(op->mem_addr)){
                pipe.data_stall = 0;
                data_load(op->mem_addr);
            }      
        }
    }

    if (pipe.data_stall > 0)
        return;
...
void pipe_stage_fetch()
{
  ...
  if (pipe.decode_op != NULL)
    ...
    if (!pipe.inst_stall){
        if (!inst_check(pipe.PC)){
            pipe.inst_stall = 1;
        }
    }

    if (l2_inst_hit_handler(pipe.PC)){
        pipe.inst_stall = 0;
        inst_load(pipe.PC);
    }      
  ...
  /*branch prediction*/
  op->predicted_pc = predict_branch(pipe.PC, &(op->predict_taken), &(op->BTBmiss));
