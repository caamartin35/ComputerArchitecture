...
void pipe_stage_mem()
{
  ...
  if (op->is_mem){
    if (data_check(op->mem_addr, pipe.time))
      pipe.data_stall = 0;
    else if (pipe.data_stall == 0){
      pipe.data_stall = 50;
    }
    else {
      pipe.data_stall--;
      if (pipe.data_stall == 0) data_load(op->mem_addr, pipe.time);
    }
  }
...
void pipe_stage_fetch()
{
  ...
  //if (pipe.inst_stall > 0){
    //pipe.inst_stall--;
    //if (pipe.inst_stall == 0) inst_load(pipe.PC);
    //}

  //reset if redirected
  //if (pipe.branch_recover) pipe.inst_stall = 0;
  ...
  //if (pipe.inst_stall == 0){
  if (inst_check(pipe.PC, pipe.time))
      pipe.inst_stall = 0;
    else if (pipe.inst_stall == 0){ 
      pipe.inst_stall = 50;
    }
    else {
      pipe.inst_stall--;
      if (pipe.inst_stall == 0) inst_load(pipe.PC, pipe.time);
    }
    //}
