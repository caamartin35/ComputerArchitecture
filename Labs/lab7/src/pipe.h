...
typedef struct Pipe_Op {
  ...
  int predict_taken;    /* whether or not the predicted was the taken or not taken*/
...
typedef struct Pipe_State {
  ...
  //address of blocks this pipe is requesting
  struct block* irequest;
  struct block* drequest;

  //a flag saying that l2 miss process has started and for it not to be cancelled, always using the started pc
  //int started;
  //uint32_t started_pc;
