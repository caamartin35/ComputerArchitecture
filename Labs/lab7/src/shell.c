...
void cycle() { 
    ...
    //cycle core0
    if (processor.core0.RUN_BIT){
      set_active_core(0);
      pipe_cycle();
    }
      
    //cycle core1
    if (processor.core1.RUN_BIT){
      set_active_core(1);
      pipe_cycle();
    }

    //cycle core2
    if (processor.core2.RUN_BIT){
      set_active_core(2);
      pipe_cycle();
    }

    //cycle core3
    if (processor.core3.RUN_BIT){
      set_active_core(3);
      pipe_cycle();
    }
...
void run(int num_cycles) {                                      
  ...
  for (i = 0; i < num_cycles; i++) {
    if (!processor.core0.RUN_BIT && !processor.core1.RUN_BIT &&
        !processor.core2.RUN_BIT && !processor.core3.RUN_BIT)
	    break;
...
void go() {
  ...
  //run the cores until they all halt
  while (processor.core0.RUN_BIT || processor.core1.RUN_BIT ||
        processor.core2.RUN_BIT || processor.core3.RUN_BIT){
...
void rdump() {
  ...
  /*printf("PC: 0x%08x\n", processor.core0.pipe.PC);

  for (i = 0; i < 32; i++) {
    printf("R%d: 0x%08x\n", i, processor.core0.pipe.REGS[i]);
  }

  printf("HI: 0x%08x\n", processor.core0.pipe.HI);
  printf("LO: 0x%08x\n", processor.core0.pipe.LO);
  printf("Cycles: %u\n", stat_cycles);
...
void get_command() {
  ...
  case 'i':
   if (scanf("%i %i", &register_no, &register_value) != 2)
      break;
   set_active_core(0); 
   core->pipe.REGS[register_no] = register_value;
   break;
...
void initialize(char *program_filename, int num_prog_files) { 
  int i;
  processor.core0.RUN_BIT = TRUE;
  processor.core1.RUN_BIT = FALSE;
  processor.core2.RUN_BIT = FALSE;
  processor.core3.RUN_BIT = FALSE;
  init_processor();
  init_memory();
