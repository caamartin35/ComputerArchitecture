...
module mips_core(/*AUTOARG*/
   ...
   stall_logic sl(.stall(stall),
		  .valids(valids),
		  .rs_num(dcd_rs),
		  .rt_num(dcd_rt),
		  .rd_num(dcd_rd));

   valid_logic vl(.rs_num(dcd_rs),
		  .rt_num(dcd_rt),
		  .rd_num(dcd_rd),
		  .valids(valids),
		  .clk(clk),
   ...
   //EXECUTE
   register rs_ex(rs_to_src1mux, rs_data, clk, ld_pipeline_regs, rst_b);
   ...
   register rt_ex(rt_to_src2mux, rt_data, clk, ld_pipeline_regs, rst_b);
...
module stall_logic(stall, valids, rs_num, rt_num, rd_num);
   ...
   always @(*) begin
      if ((~valids[rs_num] && rs_num != 0) || (~valids[rt_num] && rt_num != 0) || (~valids[rd_num] && rd_num != 0)) stall = 1;
      else stall = 0;
   end // always @ begin
endmodule // stall_logic

module valid_logic(valids, rs_num, rt_num, rd_num, clk, rst_b);

   output reg [31:0] valids;
   input [4:0] 	     rs_num;
   input [4:0] 	     rt_num;
   input [4:0] 	     rd_num;
   input 	     clk, rst_b;
   ...
   always @(*) begin
      ...
      else if(valids[rs_num] && valids[rt_num] && valids[rd_num]) begin
	 time_set[rs_num] = 3;
	 time_set[rt_num] = 3;
	 time_set[rd_num] = 3;
	 $display("time_set[rs_num]: %d", time_set[rs_num]);
